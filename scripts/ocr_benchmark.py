"""OCR benchmark: test RapidOCR on card photos.
...
def write_raw(filename: str, fragments: list[dict]):
    path = RAW_DIR / filename.replace(".jpg", ".json")
...
def main():
    ...
    for filename, card_name in CARDS:
        ...
        t0 = time.monotonic()
        fragments = sort_fragments_spatial(run_ocr_with_boxes(image_path))
        elapsed = time.monotonic() - t0
        timings.append((card_name, elapsed))
        write_raw(filename, fragments)

        ocr_text = "\n".join(f["text"] for f in fragments)
        left = wrap(ocr_text, COL)
        side_by_side(f"RapidOCR: {card_name}", left, f"Scryfall: {card_name}", right)

    conn.close()

    # Timing table
    print(f"\n{'═' * 50}")
    print(f"{'CARD':<25} {'TIME (s)':>10}")
    print(f"{'─' * 25} {'─' * 10}")
    for card, secs in timings:
        print(f"{card:<25} {secs:>10.2f}")
    print(f"{'─' * 25} {'─' * 10}")
    avg = sum(s for _, s in timings) / len(CARDS)
