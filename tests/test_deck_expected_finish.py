...
class TestSchemaIntegrity:
    ...
    def test_unique_constraint_includes_finish(self):
        """A deck can have both foil and nonfoil of the same printing in the same zone."""
        conn, db_path = _fresh_db()
        try:
            init_db(conn)
            _seed(conn)
            ...
            rows = conn.execute(
                "SELECT finish FROM deck_expected_cards WHERE deck_id = ? ORDER BY finish",
                (deck_id,),
            ).fetchall()
...
class TestMigrationV51ToV52:
    def test_existing_rows_become_nonfoil(self):
