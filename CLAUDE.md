...
- **A migration never sees the shared shadow; `init_db` takes it down for it.** The same temp views make DDL resolve to a *view*: `CREATE INDEX` answers "views may not be indexed", `ALTER TABLE` answers "Cannot add a column to a view", and `DROP VIEW IF EXISTS latest_sealed_prices` silently drops the **shadow** rather than the main-schema view, leaving the connection reading emptied tables. It is the in-place upgrade that breaks — `deploy.sh` rebuilds the image and the data volume persists, so an existing split instance runs the migration branch and the next `mtg` command crashes (de-w0v2); a freshly split one is already current. So `init_db` wraps **every** migration in `suspend_shared_shadow`, not the individual ones that happen to need it — a per-migration guard is a rule someone has to remember, and v48 is what forgetting it costs. The exceptions name themselves in `_SHADOW_TRANSPARENT_MIGRATIONS`, and a name earns a place only by reading the shared catalogue to write a local table: v50 fills `collection.card_name` from `printings`, which on a pruned instance lives only in `shared`, and suspending there would not fail — it would write NULL over the sort key. **The dispatch is the module, not a chain written beside it**: `_migrations()` discovers every `_migrate_vN_to_vN+1` by name and hard-errors on a gap or a `SCHEMA_VERSION` with nothing behind it, because a hand-written `if current < N:` line is a second copy of the same fact and its failure mode is a migration that exists and is never called. `tests/test_migration_shadow.py` replays the in-place upgrade from every version a split instance can have been left at — 41, where `mtg db split` shipped, onward.
