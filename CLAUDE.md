...
- **Never give a page or a `/static/` asset a long `max-age`.** Everything `_serve_static()` serves sits at a URL a deploy rewrites in place, so a cache holding it has no way to notice it went stale. It used to be `public, max-age=86400` with no validator: on 2026-08-25 six deploys landed and `magic.dumpster.cards` showed none of them, because a hard refresh defeats only the *browser's* cache — the request still reached Cloudflare, which had nothing to revalidate against. It took a manual purge. These now go out `no-cache` with a weak ETag, and `_write_static_response()` answers a matching `If-None-Match` with a 304, so revalidation costs headers rather than a body. The ETag is computed over the **uncompressed** bytes, so the same document keeps one validator whether or not it was gzipped. `/api/*` is `no-store` — a day-old catalogue reads exactly like a broken one. The one case that is genuinely immutable is content-addressed bytes (`_api_ingest_serve_image`, `public, max-age=86400, immutable`); keep the two distinct rather than sweeping one rule over both. Pinned by `tests/test_cache_headers.py`.
