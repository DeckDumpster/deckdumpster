...
- **Every Linux path that builds an image refuses to start on a nearly-full disk**
  (de-3ww). `bash deploy/diskcheck.sh --floor "$HOME" "${MTGC_STORE_ROOT:-$HOME}"` runs
  ahead of the build in `.github/workflows/ci.yml`, `setup.sh`, `deploy.sh` and `seed.sh`;
  `store-isolation-gate.sh` inherits it through the `setup.sh` it runs. **Both** disks are
  asked because `MTGC_STORE_ROOT` moves the container store only — `$HOME` still holds the
  uv cache the `Containerfile` bind-mounts and the default store prod's volumes live in.
  Unset, the two arguments are one filesystem and it is reported once. The floor is 10G,
  about five times what a build writes; raise it host-wide with `MTGC_DISK_FLOOR_GB` in
  `~/.config/mtgc/alerts.env` (environment beats file beats default, as with
  `MTGC_STORE_ROOT`). It fails **closed**: an unparseable floor and a path `df` cannot
  measure are refusals, not passes, because a gate that does not know the free space must
  not be the thing that says there is enough. Not wired into `mac-setup.sh` /
  `mac-deploy.sh` for the same reason the store work skips them — there the bytes land
  inside the `podman machine` VM, so a host-side `df` measures the wrong disk. This exists
  because running out of room mid-build does **not** look like a disk problem: the run that
  filled `/` reported `ld terminated with signal 7 [Bus error]`, which reads as a toolchain
  bug. `tests/test_diskcheck.py` drives the refusals and asserts the wiring — a gate nobody
  calls is the same as no gate. See `deploy/README.md` → "Refusing to build on a nearly-full
  disk".
